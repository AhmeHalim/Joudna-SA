...
class ItemDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('recommended', fn($row) => $this->renderRecommended($row))
            ->addColumn('price', fn($row) =>$row->price ?? '0.00')

            ->addColumn('image', function($row){
                return $this->renderImage($row->image, 'Items', 100);
            })
            ...
            ->rawColumns(['checkbox','actions', 'name_en', 'name_ar','image','status','recommended','price','view'])
            ->setRowId('id');
    }
    ...
    protected function renderRecommended($row)
    {
        if ($row->recommended) {
            return '<div class="badge badge-light-success">' . __('dash.yes') . '</div>';
        } else {
            return '<div class="badge badge-light-danger">' . __('dash.no') . '</div>';
        }
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('recommended')->title(__('dash.recommended'))->addClass('status-cell'),
            Column::make('price')->title(__('dash.price')),
